...
import { useDeleteEmployeeTypeMutation, useGetEmployeeTypeQuery } from 'Api/SystemConfiguration/EmployeeType';
...
import { IEmployeeTypeList } from '../Model';

const columnHelper = createColumnHelper<IEmployeeTypeList>();
...
const EmployeeTypeList = () => {
  ...
  const { data: employeeType } = useGetEmployeeTypeQuery(tableQuery)
  const [deleteItem] = useDeleteEmployeeTypeMutation()
  ...
  const columns = useMemo(() => [
    ...
    columnHelper.accessor((tableField) => tableField.name_en, {
      id: 'name_en',
      header: translate(langData, lang, LangSetUpForEmployeeType.list.name_en.key) || "Name (English)",
    }),
    columnHelper.accessor((tableField) => tableField.name_bn, {
      id: 'name_bn',
      header: translate(langData, lang, LangSetUpForEmployeeType.list.name_bn.key) || "Name (Bangla)",
    }),
        ...
        return (
          <Box className="actionField">
            <Button
              ...
              startIcon={<EditIcon />}
              LinkComponent={Link}
              href={`/admin/system-configuration/employeetype/${data.row.original.id}/edit`}
            />
            <Button
              type="button"
  ...
  const table = useReactTable({
    data: employeeType?.data || [].length === 0,
    columns,
    filterFns: {
  ...
  const breadcrumbLink = [
    ...
    { href: '/admin/system-configuration/employeetype', label: translate(langData, lang, LangSetUpForMenu.system_configuration.submodule.employeetype.key) || 'EmployeeType' }
  ]
      ...
      <Box className="contentMainField">
        ...
        <Box className="contentField">
          <Paper className="paperBody">
                ...
                <Grid item sx={{
                  ...
                  <Box className="downloadField">
                    <ul className="exportSec">
                      ...
                      <li>
                        <Button type="button" onClick={() => router.push('/admin/system-configuration/employeetype/add')} startIcon={<AddIcon />}>
                          {translate(langData, lang, LangSetUpForEmployeeType.form.add_employee_type_title.key) || "Add Employee Type"}
                        </Button>
                      </li>
                    </ul>
                  </Box>
            ...
            {/* pagination start */}
            <PaginationComponent
              paginationData={employeeType?.metaData}
