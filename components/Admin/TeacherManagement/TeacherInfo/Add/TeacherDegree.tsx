    ...
    <Card sx={{ padding: '1rem' }} >
      {fields.map((field, index) => (
        <Grid container spacing={2} key={index} >
          <Grid item md={2} >
            <FormInputLabel htmlFor="Exam Name">
              {translate(langData, lang, LangSetUpForTeacher.form.nameOfDegree.key) || "Name of Degree"}
            </FormInputLabel>
            <FormSelect name={`teacherEducations.${index}.degreeId`} size="small" id='Exam Name' dataSource={listSelectModify(currentData?.data)} />
          </Grid>
          <Grid item md={2} >
            <FormInputLabel htmlFor="Roll Number">
              {translate(langData, lang, LangSetUpForTeacher.form.department.key) || "Department"}
            </FormInputLabel>
            <FormInputBootstrap name={`teacherEducations.${index}.department`} size="small" id='Roll Number' />
          </Grid>
          <Grid item md={2} >
            <FormInputLabel htmlFor="Roll Number">
              {translate(langData, lang, LangSetUpForStudent.form.institutionName.key) || "Institute Name"}
            </FormInputLabel>
            <FormInputBootstrap name={`teacherEducations.${index}.institute_name`} size="small" id='Exam Name' />
          </Grid>
          <Grid item md={2} >
            <FormInputLabel htmlFor="Roll Number">
              {translate(langData, lang, LangSetUpForTeacher.form.boardUniversity.key) || "Board/University"}
            </FormInputLabel>
            <FormInputBootstrap name={`teacherEducations.${index}.board`} size="small" id='Exam Name' />
          </Grid>
          <Grid item md={2} >
            <FormInputLabel htmlFor="Roll Number">
              {translate(langData, lang, LangSetUpForTeacher.form.passingYear.key) || "Passing Year"}
            </FormInputLabel>
            <FormInputBootstrap name={`teacherEducations.${index}.passing_year`} size="small" id='Exam Name' />
          </Grid>
          <Grid item md={1.4} >
            <FormInputLabel htmlFor="Roll Number">
              {translate(langData, lang, LangSetUpForTeacher.form.cgpaGrade.key) || "CGPA/Grade"}
            </FormInputLabel>
