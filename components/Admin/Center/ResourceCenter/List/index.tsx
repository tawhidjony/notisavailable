...
import { useDeleteresourceLearningtypeMutation, useExportToResourceCenterExcelMutation, useExportToResourcePDFMutation, useGetResourcecenterQuery } from "Api/Center/ResourceCenter"
...
import { AddIcon, ViewIcon } from "Utils/CustomIcons"
...
import { ResourcecenterList } from "../Model"
import { panelSearchSchema } from "../Validation"
import ViewResourceCenter from "../View"
...
const columnHelper = createColumnHelper<ResourcecenterList>()
const ResourceCenterList = () => {
  ...
  const methods = useForm<IPanelSearchSchemaProp>({
    ...
    defaultValues: {
      ...
      resourcecentertype_search: { label: '', value: '' },
      code_search: "",
      name_bn_search: "",
      status_search: ""
    },
  ...
  const { data: resourcecenter, refetch } = useGetResourcecenterQuery(params)
  const [deleteItem] = useDeleteresourceLearningtypeMutation()
  ...
  const [exportToPDF] = useExportToResourcePDFMutation()
  const [exportToExcel] = useExportToResourceCenterExcelMutation();
  ...
  const panelSearchOnSubmit: SubmitHandler<IPanelSearchSchemaProp> = (items: IPanelSearchSchemaProp) => {
    ...
    let resourcecentertypeId = items?.resourcecentertype_search?.value
    let code = items?.code_search
    let name_bn = items?.name_bn_search
    let status = items?.status_search !== "" ? Number(items.status_search) : ""

    setParams((params: any) => ({ ...params, divisionId, districtId, upazilaId, citycorporationId, resourcecentertypeId, code, name_bn, status }))
    refetch()
  }
  ...
  const columns = useMemo(() => [
    columnHelper.accessor((tableField, index) => IndexSerial(params.page, params.limit, index), {
      id: 'SL',
      header: translate(langData, lang, LangSetUpForList.sl.key) || "ক্র/নং",
    }),
    ...
    columnHelper.accessor((tableField) => tableField?.resourcecentertypeId?.name_bn, {
    ...
    columnHelper.accessor((tableField) => "", {
      ...
      cell: (data) => {
        ...
        return <Box className="actionField">
          ...
          <Button type="button">
            <Link href={`/admin/center/resourcecenter/[id]/edit`} as={`/admin/center/resourcecenter/${data.row.original.id}/edit`}>
  ...
  const table = useReactTable({
    data: resourcecenter?.data || [].length === 0,
    columns,
    filterFns: {
  ...
  const onDeleteSubmit = async () => {
    await deleteItem(itemId).unwrap().then((res) => {
  ...
  const handleExportToPdf = async () => {
    const headerColumns = columns.filter(column => column.id !== "delete").map(pdf => pdf.header);
    ...
    const data = {
      headerColumns,
      totalColumns: headerColumns.length - 1,
  ...
  const breadcrumbLink: any = [
    ...
    { href: '/admin/center/resourcecenter', label: translate(langData, lang, LangSetUpForCenter.list.list.key) || 'List' },
  ]
  ...
  return (
    <Box className="table-page">
      <Box className="tableMainField" >
        ...
        <Box className="contentField">
            ...
            <List className="collapsField"
              ...
              <Box className="dropToggle">
                <Typography>
                  ...
                  {methods.watch().resourcecentertype_search?.value && (<><span>
                    {translate(langData, lang, LangSetUpForCenter.list.centerLevel.key) || "কেন্দ্রের ধরন"}
                  </span> <span className="textColor">{methods.watch('resourcecentertype_search.label')},</span></>)}
                  ...
                  <Box className="searchContent" component="form" onSubmit={methods.handleSubmit(panelSearchOnSubmit)} >
                    ...
                    >
                      ...
                      <Grid item xs={12} sm={6} md={4} lg={3} xl={3} >
                        ...
                        <FormAutocomplete name='centertype_search' dataSource={listArrayModify(resourcecentertype?.data)} />
                      </Grid>
                      <Grid item xs={12} sm={6} md={4} lg={3} xl={3} >
                        <FormInputLabel>
                          {translate(langData, lang, LangSetUpForCenter.list.centerCode.key) || "কেন্দ্রের কোড"}
                      ...
                      <Grid item xs={12} sm={6} md={4} lg={3} xl={3} >
                        ...
                        <FormSelect name='status_search' dataSource={[{ id: 0, name: "Inactive" }, { id: 1, name: "Active" }]} />
                      </Grid>
          ...
          <Paper className="paperBody">
                ...
                <Grid item sx={{
                  ...
                  <Box className="downloadField">
                    <ul className="exportSec">
                      <li>
                        <Button onClick={handleClickToPrint} type="button" className="tableBtn printBtn">
                          <PrintIcon sx={{ width: 20, marginRight: "5px" }} />
                          {translate(langData, lang, LangSetUpForList.print.key) || "Print"}
                        </Button>

                      </li>
                      <li><Button type="button" onClick={handleExportToExcel}><StickyNote2Icon sx={{ width: 20, marginRight: "5px" }} />
                        {translate(langData, lang, LangSetUpForList.export_to_excel.key) || "Export to Excel"}
                      </Button></li>
                      <li>
                        <Button onClick={handleExportToPdf} type="button">
                          <PictureAsPdfIcon sx={{ width: 20, marginRight: "5px" }} />
                          {translate(langData, lang, LangSetUpForList.export_to_pdf.key)}
                        </Button>
                      </li>
                      <li><Button type="button" onClick={() => router.push('/admin/center/resourcecenter/add')} startIcon={<AddIcon />}>
                        {translate(langData, lang, LangSetUpForMenu.center_management.submodule.resource_center.subchildmodule.add_resource_center.key) || "Add Learning center"}
                      </Button></li>
                    </ul>
            ...
            {/* pagination start */}
            <PaginationComponent
              paginationData={resourcecenter?.metaData}
      ...
      <ViewResourceCenter data={modalViewData} />
            ...
            <Box >
              <List >
                ...
                <ListItem>
                  <ListItemText><Typography component="span" className="label">
                    {translate(langData, lang, LangSetUpForCenter.list.centerLevel.key) || "কেন্দ্রের ধরন"}:
                  </Typography><Typography component="span" className="details">{modalData?.resourcecentertypeId.name_bn}</Typography></ListItemText>
                </ListItem>
                <ListItem>
                  <ListItemText><Typography component="span" className="label">
                  ...
                  <ListItemText sx={{ flexBasis: '50%' }} >
                    <Box className="singleImgField">
                      ...
                      <img
                        ...
                        src={process.env.FILE_URL + '/' + modalData?.photo}
                        alt=""
                      />
                    </Box>
                  </ListItemText>
...
export default ResourceCenterList
