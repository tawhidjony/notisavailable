...
const EditAdmission = (props: Props) => {
    ...
    const primaryAdmissionInfos = studentList?.map((data: any) => {
        ...
        if (!primaryAdmissionCheck) {
            return {
                ...
                session: data?.session?.name_bn,
                roll_number: data?.roll_number,
                learningCenter: data?.learning_center_id?.name_bn,
            }
        } else {
            return {
                studentId: data?.id,
    ...
    const primaryAdmissionSubmit = async (item: any) => {
        ...
        try {
            await createPrimaryAdmissionMultiple(modifyData).unwrap().then((res) => {
                if (res) {
                    setVisible(true);
                    setNotification({
                        autoClose: true,
    ...
    const admissionClasses = [
        { name: "১ম শ্রেণী", id: "১ম শ্রেণী" },
        { name: "২য় শ্রেণী", id: "২য় শ্রেণী" }
    ];

    const breadcumbLink = [
        { href: '#', label: 'Student Management' },
        {
            href: '/admin/student-management/add', label: 'Student List'
        }
    ];
            ...
            <Paper className="breadHeadField">
                ...
                <Box className="backBtn">
                    <Button LinkComponent={Link}
                        href="/admin/student-management/primary-school-admission">Back
                    </Button>
                </Box>
            </Paper>
                                    ...
                                    <TableBody>
                                        {fields.map((val: any, index: any) => (
                                            <TableRow key={val.id}>
                                                ...
                                                <TableCell className="TableContent">{val.learningCenter}</TableCell>
                                                <TableCell className="TableContent">{val.session}</TableCell>
                                                <TableCell className="TableContent">
                                                    <FormInputBootstrap type="text" name={`primaryAdmissions.${index}.institute_name`} placeholder='Admitted Institution Name' required />
                                                </TableCell>
                                                <TableCell className="TableContent">
                                                    <FormSelect name={`primaryAdmissions.${index}.admission_on`} defaultValue={1} size="small" id='Exam Name' dataSource={admissionClasses} required />
                                                </TableCell>
                                            </TableRow>
                                        ))}
                                    </TableBody>
                                        ...
                                        <TableRow>
                                            <TableCell colSpan={14} sx={{ border: "none", padding: "0px" }} >
                                                <Box sx={{ padding: '5px 0px', display: "flex", justifyContent: 'flex-end', alignItems: "center", gap: 1 }}  >
                                                    <Button type="submit" className="searchBtn">Update</Button>
                                                </Box>
                                            </TableCell>
                                        </TableRow>
                                    </TableFooter>
